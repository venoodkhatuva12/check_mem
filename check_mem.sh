...
if [[ "$percent" -le  $critical ]]
        then
                echo "CRITICAL - TOTALMB:- $total :: FREEMB:- $free :: Free Memory:- ($percent%)"
                exit 2
fi
if [[ "$percent" -le  $warning ]]
        then
                echo "WARNING - TOTALMB:- $total :: FREEMB:- $free :: Free Memory:- ($percent%)"
                exit 1
fi
if [[ "$percent" -gt  $warning ]]
        then
                echo "OK - TOTALMB:- $total :: FREEMB:- $free :: Free Memory:- ($percent%)"
                exit 0
fi
